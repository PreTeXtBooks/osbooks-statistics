#!/usr/bin/env python3
"""
Complete conversion of Chapter 3 CNXML files to PreTeXt format
"""
...
def main():
    ...
    # Read the existing introduction and practice sections
    existing_path = Path('pretext/source/ch03-probability-topics.ptx')
    existing_content = existing_path.read_text()
    
    # Extract introduction section (from <introduction> to </introduction>)
    intro_match = re.search(r'<introduction>.*?</introduction>', existing_content, re.DOTALL)
    intro_section = intro_match.group(0) if intro_match else ''
    
    # Extract practice section (from <section xml:id="sec-probability-topics-practice"> to end of that section)
    practice_match = re.search(r'<section xml:id="sec-probability-topics-practice">.*?</section>', existing_content, re.DOTALL)
    practice_section = practice_match.group(0) if practice_match else ''
    
    # Extract conclusion section
    conclusion_match = re.search(r'<conclusion>.*?</conclusion>', existing_content, re.DOTALL)
    conclusion_section = conclusion_match.group(0) if conclusion_match else ''
    
    # Convert each section
    print("Converting m79611 (Terminology)...")
    section1 = convert_module(
        modules_dir / 'm79611' / 'index.cnxml',
        'sec-terminology',
        'Terminology'
    )
    
    print("Converting m79612 (Independent and Mutually Exclusive Events)...")
    section2 = convert_module(
        modules_dir / 'm79612' / 'index.cnxml',
        'sec-independent-mutually-exclusive',
        'Independent and Mutually Exclusive Events'
    )
    
    print("Converting m79614 (Two Basic Rules of Probability)...")
    section3 = convert_module(
        modules_dir / 'm79614' / 'index.cnxml',
        'sec-two-basic-rules',
        'Two Basic Rules of Probability'
    )
    
    print("Converting m79616 (Contingency Tables)...")
    section4 = convert_module(
        modules_dir / 'm79616' / 'index.cnxml',
        'sec-contingency-tables',
        'Contingency Tables'
    )
    
    print("Converting m79617 (Tree and Venn Diagrams)...")
    section5 = convert_module(
        modules_dir / 'm79617' / 'index.cnxml',
        'sec-tree-venn-diagrams',
        'Tree and Venn Diagrams'
    )
    
    # Assemble final document
    output = '<?xml version="1.0" encoding="UTF-8"?>\n'
    output += '<chapter xml:id="ch03-probability-topics" xmlns:xi="http://www.w3.org/2001/XInclude">\n'
    output += '  <title>Probability Topics</title>\n\n'
    ...
    output += section5
    if practice_section:
        output += '  ' + practice_section + '\n\n'
    if conclusion_section:
        output += '  ' + conclusion_section + '\n'
    output += '</chapter>\n'
    
    # Write output
    output_path = Path('pretext/source/ch03-probability-topics.ptx')
