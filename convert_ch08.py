#!/usr/bin/env python3
"""
Complete conversion of Chapter 8 CNXML files to PreTeXt format
Converts modules for Confidence Intervals chapter
"""
    ...
    for child in elem:
        ...
        if tag == 'math':
            ...
            if effect == 'italics':
                ...
                if len(content.strip()) <= 2 or content.strip() in ['X', 'Y', 'Z', 'P', 'Q', 'x', 'y', 'z', 'p', 'q', 'k', 'n', 'EBP', 'EBM']:
...
def process_list(list_elem, namespaces, indent='    '):
    ...
    for item in list_elem.findall('.//{http://cnx.rice.edu/cnxml}item'):
        ...
        table = item.find('.//{http://cnx.rice.edu/cnxml}table')
        if table is not None:
            # Process text before table and table separately
            result += f'{indent}  <li>\n'
            ...
            text_before = ''.join(text_parts).strip()
            if text_before:
                result += f'{indent}    <p>{xml_escape(text_before)}</p>\n'
            
            # Process the table
            result += process_table(table, namespaces, indent + '    ')
            
            # Process text after the table
            if table.tail and table.tail.strip():
                result += f'{indent}    <p>{xml_escape(table.tail.strip())}</p>\n'
...
def main():
    ...
    # Convert each module
    print("Converting m79656 (A Population Proportion)...")
    section1 = convert_module(
        modules_dir / 'm79656' / 'index.cnxml',
        'sec-population-proportion',
        'A Population Proportion'
    )
    
    print("Converting m79657 (Confidence Interval - Home Costs)...")
    section2 = convert_module(
        modules_dir / 'm79657' / 'index.cnxml',
        'sec-ci-home-costs',
        'Confidence Interval (Home Costs)'
    )
    
    print("Converting m79658 (Confidence Interval - Place of Birth)...")
    section3 = convert_module(
        modules_dir / 'm79658' / 'index.cnxml',
        'sec-ci-place-of-birth',
        'Confidence Interval (Place of Birth)'
    )
    
    print("Converting m79659 (Confidence Intervals Practice)...")
    section4 = convert_module(
        modules_dir / 'm79659' / 'index.cnxml',
        'sec-confidence-intervals-practice',
        'Confidence Intervals: Practice'
    )
    
    # Assemble final document sections
    output = '<!-- Chapter 8 Converted Sections -->\n'
    output += '<!-- These sections can be inserted into ch08-confidence-intervals.ptx -->\n\n'
    output += section1
    output += section2
    output += section3
    output += section4
    ...
    # Write output
    output_path = Path('ch08_converted_sections.xml')
    output_path.write_text(output)
    
    print(f"\nConversion complete! Written to {output_path}")
    print(f"Total length: {len(output)} characters")
    print("\nThe sections are ready to be inserted into the PreTeXt chapter file.")
